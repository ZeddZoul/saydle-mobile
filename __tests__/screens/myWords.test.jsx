...
const entitled = { entitled: true, status: "active", verified: false };
...
describe("MyWords — entitlement freshness", () => {
  it("re-reads entitlement on focus, so a purchase made elsewhere unlocks it", async () => {
    // Caught on device: the tab screen stays mounted, so a mount-only fetch left
    // the paywall showing after the subscription had already started.
    const client = baseClient({ subscription: unentitled });
