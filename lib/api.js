...
export function createApiClient({
  ...
  return {
    ...
    // Cancels a pending deletion from inside the grace period. Returns the
    // user; signing in alone never cancels — see DeletionPendingCard.
    restoreMe: () => request("/api/auth/me/restore", { method: "POST" }),
    ...
    /** A voice reading the shared sample, so the picker can be auditioned. */
    voicePreview: (key) => request(`/api/voice/preview/${encodeURIComponent(key)}`),
