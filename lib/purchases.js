...
export async function ensureConfigured() {
  ...
  try {
    // Before configure, so nothing the SDK says on its way up reaches the
    // console raw.
    quietLogs(Purchases);
...
/**
 * Takes the SDK's logs out of the console and puts them behind our own tag.
 *
 * The SDK writes at DEBUG in a debug build and logs a *user cancelling a
 * purchase* at error level — `[RevenueCat] 🍎‼️ Purchase was cancelled.` React
 * Native turns any console.error into a LogBox toast, so declining a purchase
 * popped a red banner over the paywall that reads like a crash report. It is
 * not a crash, it is not an error, and it is not ours to show: cancelling is
 * the commonest outcome of showing a paywall, which is exactly why
 * `onPurchase` says nothing about it.
 *
 * Release builds are unaffected — LogBox only exists in development — so this
 * is about not making our own dev builds look broken, and about not teaching
 * anyone to ignore red banners.
 *
 * A handler rather than a log level: the message is error-level at the source,
 * so `setLogLevel(WARN)` would not hide it, and turning logging off entirely
 * would take the diagnostics with it. This keeps everything, on `console.log`,
 * where it informs without alarming.
 */
function quietLogs(Purchases) {
  try {
    Purchases.setLogHandler?.((level, message) => {
      if (__DEV__) console.log(`[RevenueCat:${level}]`, message);
    });
  } catch {
    // An older SDK without setLogHandler. Noisy logs are not worth failing a
    // purchase over.
  }
}
...
/**
 * The annual price expressed per month, or null when that is not a fair thing
 * to say.
 *
 * The whole argument for annual, and arithmetic on the store's own number
 * rather than a claim of ours. Null for anything that is not an annual
 * package, so a monthly plan never gets a redundant "that's X a month".
 *
 * Shared rather than duplicated: it lived in the onboarding paywall, and the
 * locked screen grew a second copy that quietly dropped the currency code —
 * "That's 4.17 a month" next to a price in dollars, which is at best ambiguous
 * and at worst reads as a different currency.
 */
export function monthlyEquivalent(pkg) {
  const price = pkg?.product?.price;
  if (typeof price !== "number" || pkg?.packageType !== "ANNUAL") return null;

  const currency = pkg.product.currencyCode ?? "";
  return `${currency} ${(price / 12).toFixed(2)}`.trim();
}
