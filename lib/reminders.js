...
/**
 * What the reminders step proposes before anyone touches the slider.
 *
 * One object shared by the onboarding step, the settings control and the
 * reminders hook, so "Allow and save" saves exactly what the screen showed —
 * the step used to display these defaults and then persist nothing.
 */
export const DEFAULT_REMINDER_WINDOW = { count: 3, ...DEFAULT_WINDOW };
...
export function spreadTimes(
  count,
  startTime = DEFAULT_WINDOW.start,
  endTime = DEFAULT_WINDOW.end,
) {
